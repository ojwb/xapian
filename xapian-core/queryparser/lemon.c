...
** https://www.sqlite.org/src/artifact/c1a87d15983f96851b253707985dba8783fbe41ba21ba1b76720e06c8a262206
*/
#include <stdio.h>
#include <stdarg.h>
#include <string.h>
...
void Configlist_sort(void){
  /* Cast to "char **" via "void *" to avoid aliasing problems. */
  current = (struct config*)msort((char*)current,(char**)(void*)&(current->next),
                                  Configcmp);
  currentend = 0;
  return;
}
...
void Configlist_sortbasis(void){
  /* Cast to "char **" via "void *" to avoid aliasing problems. */
  basis = (struct config*)msort((char*)current,(char**)(void*)&(current->bp),
                                Configcmp);
  basisend = 0;
  return;
}
...
static int handleflags(int i, FILE *err)
{
  ...
  if( op[j].label==0 ){
    ...
    errcnt++;
  }else if( op[j].arg==0 && op[j].func==0 ){
    /* Ignore this option */
